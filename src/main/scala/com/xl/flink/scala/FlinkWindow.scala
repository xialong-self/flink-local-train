...
//    env.getCheckpointConfig.setCheckpointingMode(CheckpointingMode.EXACTLY_ONCE)
    ...
    env.setStateBackend(new FsStateBackend("file:///D:\\C盘桌面文件\\2020-08-25Scala开始\\flink\\flinkoutput"));
    //设置checkpoint的清除策略
    ...
    val dataAddTime=
      stream.map(new RichTest)

    //
    val data_sc=dataAddTime.filter(_.ip!="0")
      .map(line=>(line.ip,1)).keyBy(0).timeWindow(Time.seconds(10)).sum(1)
